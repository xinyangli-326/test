...
const CHANNEL_RULES = {
  ...
  '朋友圈': '朋友圈：像真人发的圈，有小故事感、场景感，不硬广；条数以用户指令为准（写了“N条”就 N 条，未写默认 1 条）。',
...
const CHANNEL_FORMATS = {
  '朋友圈': '输出朋友圈文案：每条以「朋友圈①」「朋友圈②」…开头，默认每条150-250字（若用户明确指出条数/字数则严格按用户指定执行，未指明条数则输出1条），分2-3个短段落，有具体场景和小故事感，结尾一句自然引导，不硬广。只输出文案本身，不要输出任何其他分析或说明。',
...
写法：站在资深酒店从业者/行业编辑的角度，用头部酒店干货号的营销口吻输出干货（短句钩子、数字清单/步骤体、真实做法与话术、金句收尾）。
...
async function ensureFullProductIndex() {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const url = 'products_full.json?v=3' + (attempt ? ('&retry=' + attempt + '_' + Date.now()) : '');
...
function buildFocusedSystem(payload) {
  ...
  const prodBlock = prods.map(p => {
    ...
    const cmtLine = Array.isArray(p.comments) && p.comments.length
      ...
      : '';
    const visionLine = p.vision
      ? `  【商品图提炼（第一优先事实来源，直接采用其所含信息）】\n${String(p.vision).slice(0, 2200)}\n`
      : '';
    return visionLine + `- 【商品】${p.name}（${p.parent || p.type || ''}${p.cat ? '·' + p.cat : ''}）\n` +
